...
\titleformat{\subsubsection}{\vspace{-0.9em}\bfseries\small\scshape\filcenter}{\thesubsubsection}{1em}{\vspace{-0.7em}}
...
\title{\textbf{\large{How Metallicity Affects Extrasolar Planets}}}
...
\begin{abstract} 
\noindent
Many studies have aimed to identify abundance relationships and correlations between stellar metallicity and planet mass and radius for specific sizes of planets. More recently some work has attempted to compare the strength of these relationships at a discrete number of predetermined ranges, however, no undisputed studies have as yet attempted to discover the limits and turning points of these trends. In this work, the correlations between stellar metallicity and planet mass, radius and density were calculated via a Bayesian correlation analysis for a continuum of different radii, masses and densities. In this report data are presented suggesting that the effect of metallicity on a planet's mass and radius depends on the scale of the planet, with massive, large planets exhibiting a negative correlation between stellar metallicity and planet mass and radius whilst low mass, small planets exhibit a positive correlation between stellar metallicity and planet mass and radius. It is proposed that this is a result of the core dominating the mass and radius of small, low mass planets whilst the abundance of hydrogen dominates the mass and radius of larger, massive planets. No strong correlation was found for mid-sized planets.
\end{abstract}
\vspace{11mm}
\end{@twocolumnfalse} ]
...
% \begin{abstract} 
% \noindent
% % Requires fast and slow formation to happen
% \end{abstract}
% \vspace{11mm}
% \vspace{-5mm}
% \end{@twocolumnfalse} ]
...
% Not many comments on it since it was all very good
...
% Lierature figures - add thing.
% There's lots of text before a figure.
% The neptune plot might look nice - also nice thing to have something right at the start

\subsection{Previous Work}
% Is good overview of the literature and well referenced
...
\section{Theory}
% Good, and good the way I split up the planets
...
%Describe a bit more about what the MCMC statistics does + read figueira
% Definitely need more
% Means and std aresn't described what they are
...
For each pair of stellar-planet parameters; metallicity-mass, metallicity-radius and metallicity-density, the planets considered were restricted via their radii, masses and densities one at a time. The lower and upper bounds of each range were varied and for each combination a correlation analysis was performed as described prior. From this, a map of the strength of the correlation at all combinations of lower and upper bounds could be created and the values of the lower and upper bounds of the ranges where the correlation strength was maximised could be determination quantitatively.  %, of a particular parameter for a particular pair of parameters between which a correlation was being searched for.

%Above Fine

% This bit becomes unclear - peak???
% Reword/reorder
...
% Actually reference back to 1.4 to the reader

%Why the range over the third parameter

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Results}
% Good, figures are nice and text describes the data well given the amount of data that there were
...
% done a v good job at condencing the plots down into the story.
% A few sections where I don't mention the figure in the text. Need to do that
% See fig 2 - referenced in text - good. Figure 3??? Not referenced in the text.
...
Firstly, there is a positive correlation between stellar metallicity and planet mass for smaller radius planet, maximised for the range (0.5$\pm$0.1)R$_{\oplus}~\leq~$R$_{planet}~\leq~$(6.5$\pm$0.5)R$_{\oplus}$, with a mean/$\sigma$ of 4.03. % AS SHOWN IN FIGURE 3... Invites them to look at the picture, and provides clarity on which figures I'm referecngin in each section. - ADDED
...
Further trends were identified at localised intervals of mass. Of these, one negative correlation is deemed to be potentially significant, and that is for the range (275$\pm$10)M$_{\oplus}~\leq~$M$_{planet}~\leq~$(450$\pm$10)M$_{\oplus}$. The correlation in this range a mean/$\sigma$ of 2.99. This is slightly under the value needed for this to be deemed statistically significant. %, but due to the lack of error consideration in MCMC analyses, it is possible that this range does indeed have some physical significance.
% As in it may actually have a stronger correlation, or may not. Thing is though it would just increase the std and then it still wouldn't be significant. Hence why we don't actually care about the errors in MCMC analyses.
...
\end{figure}
The correlation statistics as shown in figure \ref{figure: Fe/H vs Radius correlations - Density ranges} indicate the presence of two large scale trends when restricting the planets based on their density. %, although the correlation strengths are minimal. % This comment is incongruent - and inaccurate
...
% I mean yeah obviously no low density planet of that radius is going to be able to exist. This is a bit incongruent.
...
\end{figure}
% We can probably find a way to combine the figures? - Do we need to?
...
% Interestingly the large range mentioned is similar to the range of planets for which no large density exist as shown in figure \ref{figure: Fe/H vs Radius for high density planets}. 
% This would be significant if the ranges were identical.Making the ranges identical would recover the positive correlation in low density planets
...
Firstly, there is a negative correlation between stellar metallicity and planet density for low-medium mass planets, which peaks for the range of masses (0.3$\pm$0.3)M$_{\oplus}~\leq~$M$_{planet}~\leq~$(120$\pm$8)M$_{\oplus}$, with a mean/$\sigma$ of -2.86. There is no range for which the correlation falls below -3, however the range includes a large amount of planets, as shown on figure \ref{figure: Fe/H vs Density planet plot - Small masses}. % Refinement of other parameters such as insolation may strengthen this correlation. %That's a bit of an erronous comment. I'm wondering if it's better to remove or if I'm removing too much though. Let's say something a bit more provable
...
\end{figure}
% Critique this corelation even due to the visible look.
...
\end{figure}
%Is this really needed? - Eh... nr. Oh well. I feel like I'm removing too much stuff
...
\section{Discussion}
% Try to make sure you do indeed postulate rather than saying it is. The it becomes really nice and uncomplainable.
% But is good apparently, fine as it is
...
\\\\Within each box, 4 parameters are given. From top to bottom these are: The mean of the correlation distribution, the standard deviation of the correlation distribution, the mean/$\sigma$, and the range for which this correlation was strongest. The uncertainty in the ranges are presented in the results.}
\label{table: Correlation ranges summary}
\end{table}

Small, low mass planets exhibit positive correlations between metallicity and their mass and radius. A possible explanation for this is that these low mass planets will be less likely to gain and retain a hydrogen atmosphere and so the radius and mass of the planet will be dominated by the iron and solid components. For these planets, an increase of the metallicity results in an increase of iron content seen, thus an increase in the core mass, and thus results in an increase in the overall mass of the planet. % Since for separate reasons hydrogen is not found around super-Earths (r<1.7), wont matter ahh no this is all small bois.

This explanation is supported by the models of Bitsch and Battistini (2020) \cite{Bitsch&BattistiniTheoreticalModel} which predicts an increase of planetesimal iron content, and hence core iron content, with increasing stellar metallicity.

Given this supposition, the radius may be likely to increase with stellar metallicity as well, but as it is something highly dependent on the core mass for lower mass planets, it would be expected that the strength of the correlation between stellar metallicity and planet radius is lower than the strength of the correlation between stellar metallicity and planet mass since it is a secondary effect of an increase of mass. As displayed in table \ref{table: Correlation ranges summary}, these data support this hypothesis.
...
% Taking this idea of core mass dominance on the radius, when considering high insolation of the planets, one might expect the stellar metallicity--planet mass relationship to become stronger, since this would further enforce the dominance of the core in the effect of stellar metallicity on the planets, driving the relationship, %Ohh I see. I'm thinking about the lower H so lower core. Yeah see that needs to go. Oh dear.
% and reduce the effect of the 

% I mean I could sort of say, if this then this, if this then this. No because it doesn't make sense to talk about that really.

% In the case of the stellar metallicity--planet radius relationship, one might expect the correlation to be weaker still, as hydrogen, which increases the radius more so than Fe, is less present. %Well then the points contradict. I think again I mixed up some ideas in my head
...
% Do I really need to suggest this and then give evidence or should I give evidence then suggest causes?
...
% This alone may suggest hydrogen's dominance in the relationships at these masses.\\ % I don't think this fits in with the logic now.
...
% We never did since it all got a bit confused in my head
% I doubt it would make much difference tbh. You'd need pretty high insolation to affect it. Although I suspect it would affect it a bit, pressuably the radius more than the mass.
...
An increase in the stellar metallicity would result in a decrease in the availability of hydrogen, which is a component of water, thus reducing the amount of water and producing this negative correlation between stellar metallicity and mass. %Yeah but then it wouldn't be dominated by water would it
...
% Considering the possible explanations of the high mass planets in this study, one may be able to extend the ideas to sub-Neptunes, that is, a higher metallicity could result in lower amounts of disc hydrogen which would reduce the mass of planets which large enough cores to support hydrogen. Less hydrogen would increase the debsity though... .it must be the water This would indicate that sub-Neptunes do not form via planetesimal accretion and instead must form via pebble accretion. Further studies should be conducted in this area.
% Honestly I recon it's just the high mass planets coming through. Although tbh he basically had like 8 planets. Anyone can find a correlation with 8 planets. Just gotta pick any 8. I'm guessing there's more to it than that though.

% \subsection{Evolution of Observation on Conclusions}
% Make that table of year etc.
...
% Add this ones you've uploaded it all.
...
%Unfortunetly ran out of time, but I think I have enough analysis as it is. Just need that to be quality.
